 ...
 * David
 * Version bioV4.0
 * 演示地址：http://david.biodiscover.com/index.php?r=public/biobox
 */
...
(function($) {
	var cachedata = {};
	var arrbiobox = new Array();
	var biobox = function(content,options) {
		var self 		= this;
		this.dh 		= null;
		this.mh 		= null;
		this.dc			= null;
		...
		this.initOptions = function() {
			...
			self._options.boxclass = self._options.boxclass || 'bio'+self._options.type;
		...
		this.initBox = function() {
			...
			if (self.options.height>0) {
				if(self.options.height > 1){
					...
					self.dc.css('height', self.options.height*100*$(window).height());
				}
			}
			if (self.options.width>0) {
				if(self.options.width > 1) {
					...
					self.dh.css('width', self.options.width*100+'%');
				}
			}
			self.dh.bgiframe();
		};
		...
		this.initContent = function(content) {
			...
			if (!self.options.showTitle) {
				//self.dt.hide();
				self.dh.find('.dialog-header').html('').addClass('dialog-header-line');//david
			}	
		...
		this.show = function() {
			...
			if (typeof self.options.showAnimate == "string") { //显示效果
				//self.dh.show(self.options.animate);
				self.dh.slideDown(self.options.animate);
			} else {
		...
		this.setContent = function(content) {
			...
			if (self.options.height>0) {
				...
				if(self.options.height > 1){
					...
					self.dc.css('height', self.options.height*$(window).height());
				}
			} else {
				self.dc.css('height','');
			}
		...
		this.close = function(n) {
			...
			//从数组中删除
			for(i=0;i<arrbiobox.length;i++) {
				if (arrbiobox[i].dh.get(0) == self.dh.get(0)) {
					arrbiobox.splice(i, 1);
					break;
				}
			}
			self.hide();
	...
	var bioboxs = function() {		
		var self = this;
		this._onbox = false;
		this._opening = false;
		this.zIndex = 999;
		this.length = function() {
			return arrbiobox.length;
		};
		this.open = function(content, options) {
			...
			if (options.boxid) {
				for(var i=0; i<arrbiobox.length; i++) {
					if (arrbiobox[i].dh.attr('id') == options.boxid) {
						arrbiobox[i].close();
						break;
					}
				}
			...
			self.zIndex += 10;
			var box = new biobox(content, options);
		...
		//关闭最上层窗体,程序调用方法：jQuery.bioboxs.close();
		this.close = function(){
		...
		this.getTopBox = function() {
			if (arrbiobox.length>0) {
				return arrbiobox[arrbiobox.length-1];
			} else {
				return false;
			}
		};
		$(window).scroll(function() {
			if (arrbiobox.length > 0) {
				for(i=0;i<arrbiobox.length;i++) {
					var box = arrbiobox[i];//self.getTopBox();
					/*if (box.options.position == "center") {

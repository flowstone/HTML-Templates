...
$(function(){
		...
		$(".login_wrapper").animate({ height:0 }, 500, 'ease-out',function(){
	...
	//关闭提示
	$(".tips_wrapper .close").tap(function(){
	...
	$(".content_intro").tap(function(){
		if($(this).hasClass("open")){
			$(this).children("p").animate({ height:18},500,'ease-out');
			$(this).children(".micon").animate({rotateZ: '360deg'},500,'ease-out');
			$(this).removeClass("open");
		}else
		{
			$(this).children("p").animate({ height:introHeight},500,'ease-out');
			$(this).children(".micon").animate({rotateZ: '180deg'},500,'ease-out');
			$(this).addClass("open");
		}
	})
	
	//加入收藏效果
	/*$(".set_btn .add").one("click",function(){
		$(this).addClass("hover").html("<i class='hover'></i>已收藏");
	})*/
	...
	//自动登录
	$(".login_set label,.phone_reg_pro label").live("tap",function(){
